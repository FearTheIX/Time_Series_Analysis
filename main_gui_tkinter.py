"""
Integrated Analytics Platform using Tkinter (no external GUI dependencies)
...
class IntegratedAnalyticsPlatform:
    ...
    def setup_lab1_tab(self):
        ...
        self.scraping_results = tk.Text(lab1_frame, height=15, width=100)
        scrollbar = ttk.Scrollbar(lab1_frame, orient="vertical", command=self.scraping_results.yview)
        self.scraping_results.configure(yscrollcommand=scrollbar.set)
        
        self.scraping_results.pack(side='left', fill='both', expand=True, padx=(10,0), pady=10)
    ...
    def setup_lab3_tab(self):
        ...
        self.lab3_log = tk.Text(lab3_frame, height=10, width=100)
        scrollbar = ttk.Scrollbar(lab3_frame, orient="vertical", command=self.lab3_log.yview)
        self.lab3_log.configure(yscrollcommand=scrollbar.set)
        
        self.lab3_log.pack(side='left', fill='both', expand=True, padx=(10,0), pady=10)
        scrollbar.pack(side='right', fill='y', padx=(0,10), pady=10)
    ...
    def setup_lab4_tab(self):
        ...
        self.lab4_results = tk.Text(lab4_frame, height=15, width=100)
        scrollbar = ttk.Scrollbar(lab4_frame, orient="vertical", command=self.lab4_results.yview)
        self.lab4_results.configure(yscrollcommand=scrollbar.set)
        
        self.lab4_results.pack(side='left', fill='both', expand=True, padx=(10,0), pady=10)
        scrollbar.pack(side='right', fill='y', padx=(0,10), pady=10)
    ...
    def setup_testing_tab(self):
        ...
        self.test_results = tk.Text(test_frame, height=15, width=100)
        scrollbar = ttk.Scrollbar(test_frame, orient="vertical", command=self.test_results.yview)
        self.test_results.configure(yscrollcommand=scrollbar.set)
        
        self.test_results.pack(side='left', fill='both', expand=True, padx=(10,0), pady=10)
        scrollbar.pack(side='right', fill='y', padx=(0,10), pady=10)
    
    # ... (все остальные методы остаются такими же как в оригинальном main_gui.py)
    # Методы: update_module_status, start_web_scraping, select_dataset_folder, create_annotation, 
    # split_to_xy, split_by_years, split_by_weeks, search_date, load_timeseries_data, 
    # analyze_stationarity, build_arima_model, generate_forecast, run_all_tests, 
    # test_lab1, test_lab3, test_lab4, log
